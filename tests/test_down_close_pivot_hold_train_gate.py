...
from down_close_pivot_hold_train_gate import assess
...
def test_train_gate_requires_cagr_and_sharpe_improvement():
    assert assess(cell(3, .9), cell(2, .6))["pass"]
    result = assess(cell(1, .9), cell(2, .6))
    assert not result["pass"]
    assert not result["checks"]["cagr>baseline"]
