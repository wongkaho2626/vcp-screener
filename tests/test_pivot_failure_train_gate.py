...
from pivot_failure_train_gate import assess
...
def test_train_gate_requires_improvement_and_trim_stability():
    assert assess(cell(3, .9), cell(2, .6))["pass"]
    result = assess(cell(3, .9, trim=-.1), cell(2, .6))
    assert not result["pass"]
    assert not result["checks"]["drop_top_5_expectancy>0"]
