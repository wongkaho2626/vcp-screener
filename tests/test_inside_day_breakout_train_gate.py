...
from inside_day_breakout_train_gate import assess
...
def test_train_gate_requires_profit_factor():
    assert assess(cell(3, .9), cell(2, .6))["pass"]
    result = assess(cell(3, .9, pf=1.1), cell(2, .6))
    assert not result["pass"]
    assert not result["checks"]["pf>1.2"]
