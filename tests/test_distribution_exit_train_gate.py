...
from distribution_exit_train_gate import assess
...
def test_train_gate_requires_distribution_exits_to_be_profitable():
    assert assess(cell(3, .9), cell(2, .6))["pass"]
    result = assess(cell(3, .9, exit_pf=.8), cell(2, .6))
    assert not result["pass"]
    assert not result["checks"]["distribution_exit_pf>1.0"]
