#!/usr/bin/env python3
"""Train-only gate for the frozen pivot-failure exit hypothesis."""
...
def assess(candidate: dict, baseline: dict) -> dict:
    ...
    return {"checks": checks, "pass": all(checks.values()), "open_validation": all(checks.values())}
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/pivot_failure_exit_v2/results")
    ...
    dets, dropped = filter_detections(
    ...
    baseline = compact(run_cell(
        dets, prices, entry_rule="pivot_retest", exit_rule="baseline",
        iterations=args.iterations,
    ))
    candidate = compact(run_cell(
        dets, prices, entry_rule="pivot_retest", exit_rule="pivot_failure",
        iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/pivot_failure_exit_v2/frozen_spec.md",
    ...
    path = out / f"pivot_failure_train_{stamp}.json"
