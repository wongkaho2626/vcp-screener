#!/usr/bin/env python3
"""Train-only gate for the frozen two-close breakout confirmation."""
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/two_close_breakout_v2/results")
    ...
    candidate = compact(run_cell(
        detections, prices, entry_rule="two_close_breakout",
        required_closes=2, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/two_close_breakout_v2/frozen_spec.md",
    ...
    path = out / f"two_close_breakout_train_{stamp}.json"
