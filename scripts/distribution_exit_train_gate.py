#!/usr/bin/env python3
"""Train-only gate for the frozen rolling distribution-cluster exit."""
...
def assess(candidate: dict, baseline: dict) -> dict:
    ...
    checks = {
        ...
        "distribution_exit_pf>1.0": (exit_pf or 0) > 1.0,
    }
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/distribution_exit_v2/results")
    ...
    candidate = compact(run_cell(
        ...
        exit_rule="distribution_cluster", distribution_count=3,
        distribution_window=15, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/distribution_exit_v2/frozen_spec.md",
    ...
    path = out / f"distribution_exit_train_{stamp}.json"
