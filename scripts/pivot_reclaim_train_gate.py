#!/usr/bin/env python3
"""Train-only gate for the frozen failed-breakout pivot reclaim entry."""
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/pivot_reclaim_v2/results")
    ...
    candidate = compact(run_cell(
        detections, prices, entry_rule="pivot_reclaim",
        undercut_window=15, reclaim_window=5, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/pivot_reclaim_v2/frozen_spec.md",
    ...
    path = out / f"pivot_reclaim_train_{stamp}.json"
