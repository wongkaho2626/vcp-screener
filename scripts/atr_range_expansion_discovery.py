#!/usr/bin/env python3
"""Prespecified Trial 387-393 ATR range-expansion ignition lifecycle."""
...
ATR_LOOKBACK = 20
ATR_MULTIPLE = 1.5
CLOSE_LOCATION_MIN = .75
EXIT_EMA_PERIOD = 10


def true_range_series(bars: list[dict]) -> list[float | None]:
    """Return causal daily true range, with no value before a prior close."""
    result: list[float | None] = [None] * len(bars)
    for index in range(1, len(bars)):
        high = float(bars[index].get("high") or 0)
        low = float(bars[index].get("low") or 0)
        prior_close = float(bars[index - 1].get("close") or 0)
        result[index] = max(high - low, abs(high - prior_close),
                            abs(low - prior_close))
    return result


def range_expansion_state(bars: list[dict], true_ranges: list[float | None],
                          exit_ema: list[float | None], index: int,
                          pivot: float,
                          atr_lookback: int = ATR_LOOKBACK,
                          atr_multiple: float = ATR_MULTIPLE,
                          close_location_min: float = CLOSE_LOCATION_MIN
                          ) -> dict[str, bool]:
    """Return causal bullish range-expansion and below-EMA exit states."""
    if atr_lookback <= 0 or atr_multiple <= 0:
        raise ValueError("ATR parameters must be positive")
    if not 0 <= close_location_min <= 1:
        raise ValueError("close-location threshold must be in [0, 1]")
    if index <= atr_lookback or index >= len(bars):
        return {"positive_cross": False, "negative_dominance": False}
    current_tr = true_ranges[index]
    prior_tr = true_ranges[index - atr_lookback:index]
    if current_tr is None or any(value is None for value in prior_tr):
        return {"positive_cross": False, "negative_dominance": False}
    high = float(bars[index].get("high") or 0)
    low = float(bars[index].get("low") or 0)
    ...
    prior_atr = sum(float(value) for value in prior_tr) / atr_lookback
    ema = exit_ema[index] if index < len(exit_ema) else None
    return {
        "positive_cross": bool(current_tr >= atr_multiple * prior_atr
                               and location >= close_location_min
                               and close > prior_close and close > pivot),
        "negative_dominance": bool(ema is not None and close < ema),
    }


def expansion_states(rows: list[dict], prices: dict[str, list[dict]]) -> list[dict]:
    cache = {}
    for symbol, bars in prices.items():
        if symbol != "SPY":
            closes = [float(bar.get("close") or 0) for bar in bars]
            cache[symbol] = (true_range_series(bars),
                             ema_series(closes, EXIT_EMA_PERIOD))
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        true_ranges, exit_ema = cache.get(row["symbol"], ([], []))
        state = range_expansion_state(
            bars, true_ranges, exit_ema, int(row["fill_idx"]) - 1,
...
def _write_density_failure(output: Path, stamp: str, coverage: dict,
    ...
    report = {
        ...
        "family_spec": "backtests/atr_range_expansion_v2/frozen_spec.md",
    ...
    json_path = output / f"atr_range_expansion_{stamp}.json"
    md_path = output / f"atr_range_expansion_{stamp}.md"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="backtests/atr_range_expansion_v2/results")
    ...
    train_states = expansion_states(train_rows, train_prices)
    ...
    if train_gate["passed"]:
        ...
        validation_states = expansion_states(validation_rows, validation_prices)
    ...
    report = {
        ...
        "coverage": coverage, "trials_before": TRIALS_BEFORE,
        "new_multiplicity_units": 7, "trials_after": TRIALS_AFTER,
        "parameters": {"atr_lookback": ATR_LOOKBACK,
    ...
    json_path = output / f"atr_range_expansion_{stamp}.json"
    markdown_path = output / f"atr_range_expansion_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 387–393 — ATR Range-Expansion Ignition Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            output / f"atr_range_expansion_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            output / f"atr_range_expansion_{stamp}_train_daily.csv", index=False)
    if validation_raw and validation_raw["trades"]:
        pd.DataFrame(validation_raw["trades"]).to_csv(
            output / f"atr_range_expansion_{stamp}_validation_trades.csv", index=False)
        pd.DataFrame(validation_raw["equity_curve"]).to_csv(
            output / f"atr_range_expansion_{stamp}_validation_daily.csv", index=False)
