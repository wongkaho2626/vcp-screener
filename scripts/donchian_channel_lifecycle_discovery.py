#!/usr/bin/env python3
"""Prespecified Trial 381-386 Donchian 55/20 price-channel lifecycle."""
...
def channel_state(bars: list[dict], index: int, pivot: float,
                  entry_lookback: int = ENTRY_LOOKBACK,
                  exit_lookback: int = EXIT_LOOKBACK) -> dict[str, bool]:
    """Return causal Donchian closing-high entry and closing-low exit state."""
    if entry_lookback <= 0 or exit_lookback <= 0:
        raise ValueError("channel lookbacks must be positive")
    if index < 0 or index >= len(bars):
        return {"positive_cross": False, "negative_dominance": False}
    close = float(bars[index].get("close") or 0)
    entry_start = max(0, index - entry_lookback)
    exit_start = max(0, index - exit_lookback)
    prior_entry = bars[entry_start:index]
    prior_exit = bars[exit_start:index]
    if len(prior_entry) < entry_lookback or len(prior_exit) < exit_lookback:
        return {"positive_cross": False, "negative_dominance": False}
    prior_high = max(float(bar.get("close") or 0) for bar in prior_entry)
    prior_low = min(float(bar.get("close") or 0) for bar in prior_exit)
    return {"positive_cross": close > prior_high and close > pivot,
            "negative_dominance": close < prior_low}


def channel_states(rows: list[dict], prices: dict[str, list[dict]],
                   entry_lookback: int = ENTRY_LOOKBACK,
                   exit_lookback: int = EXIT_LOOKBACK) -> list[dict]:
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        state = channel_state(bars, int(row["fill_idx"]) - 1,
...
def _write_density_failure(output: Path, stamp: str, coverage: dict,
    ...
    report = {
        ...
        "family_spec": "backtests/donchian_channel_lifecycle_v2/frozen_spec.md",
    ...
    json_path = output / f"donchian_channel_lifecycle_{stamp}.json"
    md_path = output / f"donchian_channel_lifecycle_{stamp}.md"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="backtests/donchian_channel_lifecycle_v2/results")
    ...
    train_states = channel_states(train_rows, train_prices)
    train_signals = lifecycle_signals(train_states, exit_confirm_closes=1)
    ...
    if train_gate["passed"]:
        ...
        validation_states = channel_states(validation_rows, validation_prices)
        validation_signals = lifecycle_signals(
            validation_states, exit_confirm_closes=1)
    ...
    report = {
        ...
        "coverage": coverage, "trials_before": TRIALS_BEFORE,
        "new_multiplicity_units": 6, "trials_after": TRIALS_AFTER,
        "parameters": {"entry_closing_high_lookback": ENTRY_LOOKBACK,
                       "exit_closing_low_lookback": EXIT_LOOKBACK,
                       "entry": "new 55-session closing high above pivot",
                       "exit": "new 20-session closing low",
    ...
    json_path = output / f"donchian_channel_lifecycle_{stamp}.json"
    markdown_path = output / f"donchian_channel_lifecycle_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 381–386 — Donchian 55/20 Price-Channel Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            output / f"donchian_channel_lifecycle_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            output / f"donchian_channel_lifecycle_{stamp}_train_daily.csv", index=False)
    if validation_raw and validation_raw["trades"]:
        pd.DataFrame(validation_raw["trades"]).to_csv(
            output / f"donchian_channel_lifecycle_{stamp}_validation_trades.csv", index=False)
        pd.DataFrame(validation_raw["equity_curve"]).to_csv(
            output / f"donchian_channel_lifecycle_{stamp}_validation_daily.csv", index=False)
