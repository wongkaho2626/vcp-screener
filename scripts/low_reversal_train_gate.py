#!/usr/bin/env python3
"""Train gate for adjusted-scale five-day-low reversal confirmation."""
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/low_reversal_v2/results")
    ...
    candidate = compact(run_cell(
        detections, prices, entry_rule="five_day_low_reversal",
        low_reversal_confirm_window=3, iterations=args.iterations,
    ))
    gate = assess(candidate, detection, pivot)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/low_reversal_v2/frozen_spec.md",
    ...
    path = out / f"low_reversal_train_{stamp}.json"
