#!/usr/bin/env python3
"""Train-only gate for the frozen post-breakout inside-day entry."""
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/inside_day_breakout_v2/results")
    ...
    candidate = compact(run_cell(
        detections, prices, entry_rule="inside_day_breakout",
        inside_day_window=10, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/inside_day_breakout_v2/frozen_spec.md",
    ...
    path = out / f"inside_day_breakout_train_{stamp}.json"
