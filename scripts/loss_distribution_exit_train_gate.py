#!/usr/bin/env python3
"""Train-only gate for loss-only rolling distribution-cluster exit."""
...
def assess(candidate: dict, baseline: dict) -> dict:
    ...
    exit_pf = candidate["distribution_trade_stats"].get("profit_factor")
    checks = {
        ...
        "loss_distribution_exit_pf>1.0": (exit_pf or 0) > 1.0,
    }
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/loss_distribution_exit_v2/results")
    ...
    baseline = compact(run_cell(
        detections, prices, entry_rule="down_close_pivot_hold", iterations=args.iterations,
    ))
    candidate = compact(run_cell(
        detections, prices, entry_rule="down_close_pivot_hold",
        exit_rule="loss_distribution_cluster", distribution_count=3,
        distribution_window=15, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/loss_distribution_exit_v2/frozen_spec.md",
        "train_period": [START, END], "validation_accessed": False,
        "trials_declared": TRIALS_DECLARED, "coverage": coverage,
        "membership_drops": dropped, "baseline": baseline,
        "candidate": candidate, "train_gate": gate,
    }
    out = Path(args.output_dir); out.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y-%m-%d_%H%M%S")
    path = out / f"loss_distribution_exit_train_{stamp}.json"
