#!/usr/bin/env python3
"""Prespecified Trial 345-351 volatility squeeze-release VCP lifecycle."""
...
def band_series(bars: list[dict], period: int = 20) -> list[dict | None]:
    """Causal Bollinger midpoint and normalized full bandwidth per close."""
    if period <= 1:
        raise ValueError("band period must exceed one")
    result: list[dict | None] = []
    for index in range(len(bars)):
        if index - period + 1 < 0:
            result.append(None)
            continue
        closes = [float(row.get("close") or 0)
                  for row in bars[index - period + 1:index + 1]]
        if any(value <= 0 for value in closes):
            result.append(None)
            continue
        midpoint = statistics.fmean(closes)
        deviation = statistics.pstdev(closes)
        result.append({"midpoint": midpoint,
                       "bandwidth": 4 * deviation / midpoint})
    return result


def release_state(bars: list[dict], bands: list[dict | None], index: int,
                  pivot: float, reference_lookback: int = 126,
                  squeeze_percentile: float = 20) -> dict:
    """Return causal squeeze-release and middle-band exit state."""
    if not 0 < squeeze_percentile < 100 or reference_lookback <= 0:
        raise ValueError("invalid squeeze parameters")
    if index <= 0 or index >= len(bars) or index >= len(bands):
        return {"release": False, "below_mid": False}
    current = bands[index]
    prior = bands[index - 1]
    reference = bands[index - reference_lookback:index]
    if (current is None or prior is None or len(reference) < reference_lookback
            or any(value is None for value in reference)):
        return {"release": False,
                "below_mid": bool(current and float(bars[index]["close"]) < current["midpoint"])}
    threshold = float(np.percentile(
        [float(value["bandwidth"]) for value in reference], squeeze_percentile,
    ))
    close = float(bars[index].get("close") or 0)
    prior_close = float(bars[index - 1].get("close") or 0)
    release = (float(prior["bandwidth"]) <= threshold
               and float(current["bandwidth"]) > float(prior["bandwidth"])
               and close > prior_close and close > pivot)
    return {"release": release, "below_mid": close < float(current["midpoint"])}


def squeeze_states(rows: list[dict], prices: dict[str, list[dict]],
                   band_period: int = 20, reference_lookback: int = 126,
                   squeeze_percentile: float = 20) -> list[dict]:
    """Attach the frozen causal squeeze-release state to daily setup rows."""
    cache = {symbol: band_series(bars, band_period)
    ...
    for row in rows:
        ...
        state = release_state(
            bars, cache.get(row["symbol"]) or [], index,
            float(row.get("pivot") or 0), reference_lookback,
            squeeze_percentile,
        )
...
def lifecycle_signals(states: list[dict], max_attempts: int = 3) -> list[dict]:
    """Emit independent squeeze releases and next-open SMA20 exits."""
    if max_attempts <= 0:
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/squeeze_release_lifecycle_v2/results")
    ...
    if train_gate["passed"]:
        ...
        holdout_states = squeeze_states(holdout_rows, holdout_prices)
        ...
        holdout = {"candidate_rows": len(holdout_rows),
                   "squeeze_states": len(holdout_states),
    ...
    report = {
        ...
        "family_spec": "backtests/squeeze_release_lifecycle_v2/frozen_spec.md",
        ...
        "trials_after": 351,
        "parameters": {"band_period": 20,
                       "reference_lookback_sessions": 126,
                       "squeeze_percentile": 20,
                       "entry_requires_bandwidth_expansion": True,
                       "entry_requires_up_close_above_pivot": True,
                       "exit_below_sma20": True,
                       "max_attempts": 3,
                       "max_hold_sessions": 60},
        "membership_drops": {"train": train_drops, "holdout": holdout_drops},
        "train": {"candidate_rows": len(train_rows),
                  "squeeze_states": len(train_states),
    ...
    jp = out / f"squeeze_release_lifecycle_{stamp}.json"
    mp = out / f"squeeze_release_lifecycle_{stamp}.md"
    jp.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 345–351 — Volatility Squeeze-Release VCP Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            out / f"squeeze_release_lifecycle_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            out / f"squeeze_release_lifecycle_{stamp}_train_daily.csv", index=False)
    if holdout_raw and holdout_raw["trades"]:
        pd.DataFrame(holdout_raw["trades"]).to_csv(
            out / f"squeeze_release_lifecycle_{stamp}_holdout_trades.csv", index=False)
        pd.DataFrame(holdout_raw["equity_curve"]).to_csv(
            out / f"squeeze_release_lifecycle_{stamp}_holdout_daily.csv", index=False)
