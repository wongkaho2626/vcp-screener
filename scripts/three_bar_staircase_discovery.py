#!/usr/bin/env python3
"""Prespecified Trial 394-399 three-bar price-staircase lifecycle."""
...
def staircase_state(bars: list[dict], index: int,
                    pivot: float) -> dict[str, bool]:
    """Return causal three-bar staircase entry and two-low failure exit."""
    ...
    close = closes[-1]
    positive = (closes[0] < closes[1] < closes[2]
                and lows[0] < lows[1] < lows[2]
                and close > pivot)
    prior_lows = [float(bars[offset].get("low") or 0)
                  for offset in (index - 2, index - 1)]
    negative = close < min(prior_lows)
    return {"positive_cross": positive, "negative_dominance": negative}


def staircase_states(rows: list[dict],
                     prices: dict[str, list[dict]]) -> list[dict]:
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        state = staircase_state(bars, int(row["fill_idx"]) - 1,
...
def _write_density_failure(output: Path, stamp: str, coverage: dict,
    ...
    report = {
        ...
        "family_spec": "backtests/three_bar_staircase_v2/frozen_spec.md",
    ...
    json_path = output / f"three_bar_staircase_{stamp}.json"
    md_path = output / f"three_bar_staircase_{stamp}.md"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="backtests/three_bar_staircase_v2/results")
    ...
    train_states = staircase_states(train_rows, train_prices)
    ...
    if train_gate["passed"]:
        ...
        validation_states = staircase_states(validation_rows, validation_prices)
    ...
    report = {
        ...
        "coverage": coverage, "trials_before": TRIALS_BEFORE,
        "new_multiplicity_units": 6, "trials_after": TRIALS_AFTER,
        "parameters": {"staircase_sessions": 3,
                       "entry": "three rising closes and lows above pivot",
                       "exit": "close below both preceding session lows",
    ...
    json_path = output / f"three_bar_staircase_{stamp}.json"
    markdown_path = output / f"three_bar_staircase_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 394–399 — Three-Bar Price Staircase Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            output / f"three_bar_staircase_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            output / f"three_bar_staircase_{stamp}_train_daily.csv", index=False)
    if validation_raw and validation_raw["trades"]:
        pd.DataFrame(validation_raw["trades"]).to_csv(
            output / f"three_bar_staircase_{stamp}_validation_trades.csv", index=False)
        pd.DataFrame(validation_raw["equity_curve"]).to_csv(
            output / f"three_bar_staircase_{stamp}_validation_daily.csv", index=False)
