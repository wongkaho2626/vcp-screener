#!/usr/bin/env python3
"""Prespecified Trial 448-454 gap-adjusted intraday lifecycle."""
...
def return_decomposition(bars: list[dict], index: int,
                         period: int = RETURN_PERIOD,
                         ) -> tuple[float, float] | None:
    """Return causal regular-session and overnight log-return sums."""
    if period <= 0:
    ...
    for offset in range(start, index + 1):
        open_price = float(bars[offset].get("open") or 0)
        close = float(bars[offset].get("close") or 0)
        prior_close = float(bars[offset - 1].get("close") or 0)
        if min(open_price, close, prior_close) <= 0:
            return None
        intraday += math.log(close / open_price)
        overnight += math.log(open_price / prior_close)
    return intraday, overnight


def followthrough_state(bars: list[dict], index: int, pivot: float,
                        period: int = RETURN_PERIOD,
                        ) -> dict[str, bool | float | None]:
    """Return causal intraday-led entry and intraday-failure exit state."""
    values = return_decomposition(bars, index, period)
    if values is None:
        return {"positive_cross": False, "negative_dominance": False,
                "intraday_log_sum": None, "overnight_log_sum": None}
    intraday, overnight = values
    close = float(bars[index].get("close") or 0)
    return {"positive_cross": intraday > 0 and intraday > overnight
                              and close > pivot,
            "negative_dominance": intraday <= 0,
            "intraday_log_sum": intraday,
            "overnight_log_sum": overnight}


def followthrough_states(rows: list[dict],
                         prices: dict[str, list[dict]]) -> list[dict]:
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        state = followthrough_state(bars, int(row["fill_idx"]) - 1,
...
def _write_density_failure(output: Path, stamp: str, coverage: dict,
    ...
    report = {
        ...
        "family_spec": "backtests/intraday_followthrough_lifecycle_v2/frozen_spec.md",
    ...
    json_path = output / f"intraday_followthrough_lifecycle_{stamp}.json"
    md_path = output / f"intraday_followthrough_lifecycle_{stamp}.md"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir",
                        default="backtests/intraday_followthrough_lifecycle_v2/results")
    ...
    train_states = followthrough_states(train_rows, train_prices)
    ...
    if train_gate["passed"]:
        ...
        validation_states = followthrough_states(validation_rows, validation_prices)
    ...
    report = {
        ...
        "parameters": {"return_sessions": RETURN_PERIOD,
                       "entry": "positive intraday sum above overnight sum and pivot",
                       "exit": "non-positive intraday sum",
    ...
    json_path = output / f"intraday_followthrough_lifecycle_{stamp}.json"
    markdown_path = output / f"intraday_followthrough_lifecycle_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 448–454 — Gap-Adjusted Intraday Follow-Through Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            output / f"intraday_followthrough_lifecycle_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            output / f"intraday_followthrough_lifecycle_{stamp}_train_daily.csv", index=False)
    if validation_raw and validation_raw["trades"]:
        pd.DataFrame(validation_raw["trades"]).to_csv(
            output / f"intraday_followthrough_lifecycle_{stamp}_validation_trades.csv", index=False)
        pd.DataFrame(validation_raw["equity_curve"]).to_csv(
            output / f"intraday_followthrough_lifecycle_{stamp}_validation_daily.csv", index=False)
