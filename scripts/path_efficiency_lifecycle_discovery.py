#!/usr/bin/env python3
"""Prespecified Trial 334-339 signed path-efficiency VCP lifecycle."""
...
def signed_efficiency(bars: list[dict], index: int, period: int = 10) -> float | None:
    """Signed endpoint change divided by the causal absolute close path."""
    if period <= 0 or index - period < 0:
        return None
    closes = [float(bars[i].get("close") or 0)
...
def sma(bars: list[dict], index: int, period: int = 20) -> float | None:
    ...
    return sum(closes) / period if all(value > 0 for value in closes) else None


def efficiency_states(rows: list[dict], prices: dict[str, list[dict]],
                      efficiency_period: int = 10,
                      sma_period: int = 20) -> list[dict]:
    """Attach causal efficiency, pivot and SMA state to daily setup rows."""
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        signal_index = int(row["fill_idx"]) - 1
        efficiency = signed_efficiency(bars, signal_index, efficiency_period)
        average = sma(bars, signal_index, sma_period)
        close = float(row.get("close") or 0)
        if efficiency is None or average is None or close <= 0:
            continue
        states.append({**row, "efficiency": efficiency,
                       "above_sma20": close > average,
                       "above_pivot": close > float(row.get("pivot") or 0)})
    return states


def lifecycle_signals(states: list[dict], entry_threshold: float = .30,
                      max_attempts: int = 3) -> list[dict]:
    """Emit fresh positive-efficiency cycles with next-open state exits."""
    if not 0 < entry_threshold < 1 or max_attempts <= 0:
        raise ValueError("invalid path-efficiency lifecycle parameters")
    ...
    for setup_rows in by_setup.values():
        ...
        while cursor < len(ordered) and attempts < max_attempts:
            entry_pos = next((index for index in range(cursor, len(ordered))
                              if ordered[index - 1]["efficiency"] <= entry_threshold
                              and ordered[index]["efficiency"] > entry_threshold
                              and ordered[index]["above_sma20"]
            ...
            exit_pos = next((index for index in range(entry_pos + 1, len(ordered))
                             if ordered[index]["efficiency"] <= 0
                             or not ordered[index]["above_sma20"]), None)
            attempts += 1
            ...
            signal.update({"attempt": attempts,
                           "signed_efficiency_10": entry["efficiency"]})
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/path_efficiency_lifecycle_v2/results")
    ...
    if train_gate["passed"]:
        ...
        holdout_states = efficiency_states(holdout_rows, holdout_prices)
        ...
        holdout = {"candidate_rows": len(holdout_rows),
                   "efficiency_states": len(holdout_states),
    ...
    report = {
        ...
        "family_spec": "backtests/path_efficiency_lifecycle_v2/frozen_spec.md",
        ...
        "trials_after": 339,
        "parameters": {"efficiency_sessions": 10,
                       "entry_efficiency": .30,
                       "entry_requires_above_pivot": True,
                       "entry_requires_above_sma20": True,
                       "exit_efficiency": 0.0,
        ...
        "train": {"candidate_rows": len(train_rows),
                  "efficiency_states": len(train_states),
    ...
    jp = out / f"path_efficiency_lifecycle_{stamp}.json"
    mp = out / f"path_efficiency_lifecycle_{stamp}.md"
    jp.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 334–339 — Signed Path-Efficiency VCP Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            out / f"path_efficiency_lifecycle_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            out / f"path_efficiency_lifecycle_{stamp}_train_daily.csv", index=False)
    if holdout_raw and holdout_raw["trades"]:
        pd.DataFrame(holdout_raw["trades"]).to_csv(
            out / f"path_efficiency_lifecycle_{stamp}_holdout_trades.csv", index=False)
        pd.DataFrame(holdout_raw["equity_curve"]).to_csv(
            out / f"path_efficiency_lifecycle_{stamp}_holdout_daily.csv", index=False)
