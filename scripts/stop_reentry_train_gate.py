#!/usr/bin/env python3
"""Train-only gate for one-time pivot-reclaim re-entry after a stopout."""
...
def assess(candidate: dict, baseline: dict) -> dict:
    ...
    reentry_pf = candidate["reentry_trade_stats"].get("profit_factor")
    checks = {
        ...
        "reentry_pf>1.0": (reentry_pf or 0) > 1.0,
    }
...
def main() -> None:
    ...
    ap.add_argument("--output-dir", default="backtests/stop_reentry_v2/results")
    ...
    candidate = compact(run_cell(
        detections, prices, entry_rule="down_close_stop_reentry",
        reentry_window=20, iterations=args.iterations,
    ))
    gate = assess(candidate, baseline)
    report = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "frozen_spec": "backtests/stop_reentry_v2/frozen_spec.md",
    ...
    path = out / f"stop_reentry_train_{stamp}.json"
