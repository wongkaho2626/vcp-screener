#!/usr/bin/env python3
"""Prespecified Trial 425-431 Aroon extreme-recency lifecycle."""
...
def aroon_values(bars: list[dict], index: int,
                 period: int = AROON_PERIOD) -> tuple[float, float] | None:
    """Return causal Aroon Up/Down using the most recent tied extremes."""
    if period <= 1:
        raise ValueError("Aroon period must exceed one")
    ...
    highest = max(highs)
    lowest = min(lows)
    latest_high = max(offset for offset, value in enumerate(highs)
                      if value == highest)
    latest_low = max(offset for offset, value in enumerate(lows)
                     if value == lowest)
    sessions_since_high = period - 1 - latest_high
    sessions_since_low = period - 1 - latest_low
    aroon_up = 100.0 * (period - sessions_since_high) / period
    aroon_down = 100.0 * (period - sessions_since_low) / period
    return aroon_up, aroon_down


def aroon_state(bars: list[dict], index: int, pivot: float,
                period: int = AROON_PERIOD,
                entry_up: float = ENTRY_UP,
                entry_down: float = ENTRY_DOWN
                ) -> dict[str, bool | float | None]:
    """Return causal Aroon entry and reverse-recency failure state."""
    if not 0 <= entry_down < entry_up <= 100:
        raise ValueError("invalid Aroon entry thresholds")
    values = aroon_values(bars, index, period)
    if values is None:
        return {"positive_cross": False, "negative_dominance": False,
                "aroon_up": None, "aroon_down": None}
    aroon_up, aroon_down = values
    close = float(bars[index].get("close") or 0)
    qualifies = (aroon_up >= entry_up and aroon_down <= entry_down
                 and aroon_up > aroon_down and close > pivot)
    return {"positive_cross": qualifies,
            "negative_dominance": aroon_up < aroon_down,
            "aroon_up": aroon_up, "aroon_down": aroon_down}


def aroon_states(rows: list[dict],
                 prices: dict[str, list[dict]]) -> list[dict]:
    states = []
    for row in rows:
        bars = prices.get(row["symbol"]) or []
        state = aroon_state(bars, int(row["fill_idx"]) - 1,
...
def _write_density_failure(output: Path, stamp: str, coverage: dict,
    ...
    report = {
        ...
        "family_spec": "backtests/aroon_recency_lifecycle_v2/frozen_spec.md",
    ...
    json_path = output / f"aroon_recency_lifecycle_{stamp}.json"
    md_path = output / f"aroon_recency_lifecycle_{stamp}.md"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir",
                        default="backtests/aroon_recency_lifecycle_v2/results")
    ...
    train_states = aroon_states(train_rows, train_prices)
    ...
    if train_gate["passed"]:
        ...
        validation_states = aroon_states(validation_rows, validation_prices)
    ...
    report = {
        ...
        "coverage": coverage, "trials_before": TRIALS_BEFORE,
        "new_multiplicity_units": 7, "trials_after": TRIALS_AFTER,
        "parameters": {"aroon_sessions": AROON_PERIOD,
                       "entry_aroon_up_min": ENTRY_UP,
                       "entry_aroon_down_max": ENTRY_DOWN,
                       "entry": "Aroon recent-high dominance above pivot",
                       "exit_confirm_reverse_recency_closes": 2,
    ...
    json_path = output / f"aroon_recency_lifecycle_{stamp}.json"
    markdown_path = output / f"aroon_recency_lifecycle_{stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, allow_nan=False) + "\n")
    adjusted = (train_cell.get("robustness") or {}).get("risk_adjusted") or {}
    lines = ["# Trial 425–431 — Aroon Extreme-Recency Lifecycle", "",
    ...
    if train_raw["trades"]:
        pd.DataFrame(train_raw["trades"]).to_csv(
            output / f"aroon_recency_lifecycle_{stamp}_train_trades.csv", index=False)
        pd.DataFrame(train_raw["equity_curve"]).to_csv(
            output / f"aroon_recency_lifecycle_{stamp}_train_daily.csv", index=False)
    if validation_raw and validation_raw["trades"]:
        pd.DataFrame(validation_raw["trades"]).to_csv(
            output / f"aroon_recency_lifecycle_{stamp}_validation_trades.csv", index=False)
        pd.DataFrame(validation_raw["equity_curve"]).to_csv(
            output / f"aroon_recency_lifecycle_{stamp}_validation_daily.csv", index=False)
